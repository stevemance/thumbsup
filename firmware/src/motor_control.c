...
static uint16_t speed_to_pulse(int8_t speed) {
    int16_t range = PWM_MAX_PULSE - PWM_MIN_PULSE;
    int16_t mid_pulse = PWM_NEUTRAL_PULSE;

    if (speed == 0) {
        return mid_pulse;
    } else if (speed > 0) {
        return mid_pulse + (speed * (PWM_MAX_PULSE - mid_pulse)) / 100;
    } else {
        return mid_pulse + (speed * (mid_pulse - PWM_MIN_PULSE)) / 100;
    }
}
...
bool motor_control_init(void) {
    ...
    for (int i = 0; i < MOTOR_COUNT; i++) {
        ...
        motor_control_set_pulse(i, PWM_NEUTRAL_PULSE);
