...
for ph, x0 in CELL.items():
    ...
    EXPLICIT[ls] = (x0 + 3.0, 9.3, 90, T, f"phase {ph} low side: drain tab faces the phase strip at the front, source pins face its shunt {rs} directly behind; hot part, top")
    EXPLICIT[hs] = (x0 + 9.2, 9.5, 270, T, f"phase {ph} high side: source pins face the phase strip at the front, drain tab (VBAT) faces its 10 uF {cap} behind; hot part, top")
    EXPLICIT[jw] = (x0 + 6.1, 3.0, 0, T, f"phase {ph} motor wire: at the front edge (drum side), directly in front of the phase strip joining {hs} source and {ls} drain")
    EXPLICIT[rs] = (x0 + 3.2, 15.6, 0, T, f"phase {ph} shunt: directly behind {ls}'s source pins (pad 1), GND pad 2 toward the cell centre next to {cap}'s GND pad; 0.8 W peak, top")
    EXPLICIT[cap] = (x0 + 8.6, 15.75, 270, T, f"phase {ph} bridge 10 uF: VBAT pad on {hs}'s drain tab, GND pad beside {rs}'s GND pad: closes the commutation loop on L1 (DESIGN 6.1)")
...
EXPLICIT.update({
    ...
    "MH4": (BW - 3.0, 32.0, 0, T, "M2 stack/chassis hole, rear-right corner"),
    ...
    "Q7": (10.2, 9.0, 180, T, "inrush FET: drain tab faces JBAT1, source pins face Q8's (common source PSW_S between them); 16 W peak at switch-on, top"),
    "Q8": (17.7, 9.0, 0, T, "reverse FET: source pins face Q7's, drain tab (VBAT_SW) faces RS4; ~1 W at 22 A, top"),
    "RS4": (26.1, 9.0, 0, T, "pack shunt: pad 1 on Q8's drain tab, pad 2 (VBAT) toward C1/D1; INA239 Kelvin taps from the pad inner edges"),
    "D1": (27.0, 3.0, 0, T, "bus TVS: in the front strip right in front of RS4's VBAT pad, a few mm from C1's VBAT pad and the bridge's left end"),
    ...
    "R302": (15.5, 18.5, 90, T, "drive L VM feed 0.1R: pad 1 at C1's VBAT pad, pad 2 toward U3's VM pins; own branch off C1, not through the bridge copper (DESIGN 6.6); up to 1 W, top, away from U3's thermal pad"),
    "JL1": (7.6, 32.4, 0, T, "drive L phase A wire: rear edge, left end (left motor behind)"),
    "JL2": (11.8, 32.4, 0, T, "drive L phase B wire: rear edge"),
    "JL3": (16.0, 32.4, 0, T, "drive L phase C wire: rear edge, next to U3's outputs"),
    ...
    "L1": (56.5, 30.0, 0, T, "buck inductor: SW pad (left) straight behind U2's SW pin 45, +5V pad (right) over the output caps; 4 mm tall, warm, top"),
    "D2": (51.0, 30.85, 270, T, "buck catch diode: cathode (top) beside L1's SW pad, anode (bottom) next to C29's GND pad; 0.18 W, top"),
    "C29": (54.6, 33.85, 180, T, "buck output cap: GND pad beside D2's anode, +5V pad under L1: closes the L1/D2/C29 loop in ~7 x 5 mm (DESIGN 6.7)"),
    "C30": (58.3, 33.85, 0, T, "second buck output cap: +5V pad under L1's +5V pad"),
    "C27": (59.6, 26.0, 0, T, "buck VIN cap: VIN pad ~3 mm from U2 pin 47 at U2's rear-right corner, GND pad toward the buck ground (LMR16006: CIN first)"),
    # drive right (rear-right, the right motor is behind this corner)
    "U4": (67.00, 24.5, 90, T, "drive R DRV8316: rotated 90 so the phase outputs face the JR wire holes on the right edge and the VM pins face R402 behind it; CSA/logic pins via to the MCU below; ~1-2 W, top"),
    ...
    "JR1": (73.10, 18.2, 0, T, "drive R phase A wire: right edge beside U4's outputs (right motor behind this corner)"),
    "JR2": (73.10, 22.0, 0, T, "drive R phase B wire: right edge beside U4's outputs"),
    "JR3": (73.10, 25.8, 0, T, "drive R phase C wire: right edge beside U4's outputs, clear of MH4"),
    "TH1": (58.73, 15.6, 90, T, "weapon FET NTC: in the gap behind phase B's high-side drain tab (the hottest copper), between C26 and RS1 (DESIGN 3.2)"),
    # bottom: header and sensor connectors
    "J1": (56.0, 24.5, 0, B, "header to the compute board (bottom): beside the MCU, rear half, behind the bridge's return region (the pack current flows along the front strip and out at the left, DESIGN 6.2)"),
    "J2": (22.7, 31.9, 180, B, "drive L sensor connector (bottom): rear edge under U3 (the left motor is behind), mating face at the edge; 3.35 mm tall: the compute board keeps this corner clear"),
    "J3": (64.0, 31.9, 180, B, "drive R sensor connector (bottom): rear edge under R402 next to U4 (the right motor is behind), mating face at the edge; 3.35 mm tall"),
})
...
# ---------------------------------------------------------------- top: bridge extras
add("NT1", T, ("RS1", "2"), "Kelvin tie SNA: at RS1's GND pad inner edge (DESIGN 6.3)")
add("NT2", T, ("RS2", "2"), "Kelvin tie SNB: at RS2's GND pad inner edge")
add("NT3", T, ("RS3", "2"), "Kelvin tie SNC: at RS3's GND pad inner edge")
...
add("C28", T, ("U2", "44"), "bootstrap cap CB-SW at pins 44/45")
...
# ---------------------------------------------------------------- top: user-facing
add("D3", T, (34.0, 33.8), "power LED: rear edge, visible from above/behind (DESIGN 3.1: visible from outside)")
...
add("TP9", T, ("U2", "28"), "W_nFAULT test pad: at U2 nFAULT")
add("TP8", T, ("U3", "21"), "DRV_OFF test pad: at U3 DRVOFF")

# ---------------------------------------------------------------- bottom: MCU and its parts
add("U1", B, (41.0, 26.0), "STM32G474 (bottom): cool (~0.26 W); central rear, between the drive ICs and the weapon driver, away from the pack current (DESIGN 6.2); vias to U2/U3/U4 logic")
...
add("U6", B, (48.5, 14.5), "interlock AND gates (bottom): under the bridge's rear strip (VBAT/GND copper above, no switching node), between U2's INL pins, the MCU's CHxN pins and J1's W_ARM end; digital enables, not noise-critical")
...
for s, u, r in (("30", "U3", "R302"), ("40", "U4", "R402")):
    ...
    add(f"R{s}0 C{s}7", B, (u, "5"), f"{u} buck resistor-mode parts (bottom, under SW_BK/FB_BK): ~0.05 W only until firmware sets BUCK_DIS")
...
add("U8", B, ("J4", "3"), "BQ76907 (bottom, cool, 1 mm): under J4, cell taps short")
add("R6 R7 R8 R9 R10 R11", B, ("J4", "3"), "cell-input series R: between J4 and U8 (DESIGN 6.9: at U8)")
...
SW_AT = {"L": (14.5, 28.5), "R": (57.5, 31.5)}
for s, j, u, v, jp, d, n0 in (("L", "J2", "U9", "U11", "JP1", "D7", 0), ("R", "J3", "U10", "U12", "JP2", "D8", 1)):
    add(v, B, SW_AT[s], f"sensor {s} supply switch: toward {j} pin 1 (VS) but ~10 mm from the drive IC (85 C part, DESIGN 6.5)")
    add(jp, B, (v, "5"), f"sensor {s} supply select jumper at the switch input (bottom: reachable with the stack apart)")
    add(u, B, (j, "4"), f"sensor {s} Schmitt buffer: next to {j}")
    add(d, B, (j, "6"), f"motor {s} NTC clamp at {j} pin 6")
...
for s, (pu, ser, flt, ntc, sw, dec) in hr.items():
    ...
    add(ser, B, (u, "1"), f"sensor {s} line series R between {j} and {u}")
    add(flt, B, (u, "3"), f"sensor {s} line filter caps (DNP) at {u}'s inputs")
    add(pu, B, (j, "4"), f"sensor {s} pull-ups at {j}")
    add(ntc, B, (j, "6"), f"motor {s} NTC pull-up / series R / filter near {j} pin 6")
    add(sw, B, (v, "5"), f"{v} input/output caps at the switch")
    add(dec, B, (u, "8"), f"{u} decoupling")
...
FIRST = ("U1 C60 C61 C62 C63 C64 C65 R60 C66 C71 C74 C67 C80 C81 C82 C90 C91 C92 C41 C42 C43 C19 "
         "C302 C308 C309 C310 C402 C408 C409 C410 R300 C307 R400 C407 "
         "U8 C11 C9 U7 C3 C2 U6 C40 U14 C17 U12 C48 C49 JP2 U10 C50 D8 R114 R115 R116 C114 C115 C116 R57 R58 R59 R53 R117 C73 C48 C49 C50 "
